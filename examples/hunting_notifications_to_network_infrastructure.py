...
class HuntingNotificationToNetworkInfrastructureHandler:
    ...
    def __init__(self, apikey):
        ...
        self.networking_infrastructure = defaultdict(lambda: defaultdict(lambda: {}))
    ...
    async def get_network_infrastructure(self):
        ...
        while True:
            ...
            await self.queue.put(
                {"contacted_addresses": contacted_ips, "type": "ips", "file": hash}
            )
            await self.queue.put(
                {"contacted_addresses": contacted_urls, "type": "urls", "file": hash}
            )
                    ...
                    if network_inf[1]:
                        ...
                        for address in network_inf[1]:
                            if address["type"] in ("domain", "ip_address"):
                                ...
                                print(
                                    "\t\t{}".format(
                                        address["context_attributes"]["url"]
                                    )
                                )
...
async def main():
    ...
    limit_datetime = datetime.datetime.combine(limit_date, datetime.datetime.min.time())
    ...
    enqueue_files_task = loop.create_task(
        handler.get_hunting_notification_files(args.filter, timestamp_to_compare, limit)
    )
    network_inf_task = loop.create_task(handler.get_network_infrastructure())
    build_network_inf_task = loop.create_task(handler.build_network_infrastructure())
