...
class VTISearchToNetworkInfrastructureHandler:
    ...
    async def get_network(self):
        ...
        while True:
            ...
            await self.queue.put(
                {"contacted_addresses": contacted_ips, "type": "ips", "file": checksum}
            )
            await self.queue.put(
                {
                ...
                for inf in item[1].items():
                    for key in inf[1]:
                        print(
                            "{}: {}".format(
                                key["type"].upper(),
                                key.get("context_attributes", {}).get("url")
                                or key.get("id"),
                            )
                        )
...
async def main():
    ...
    parser.add_argument("--query", required=True, help="VT Intelligence search query")
    ...
    try:
        ...
        build_network_task = loop.create_task(handler.build_network())
