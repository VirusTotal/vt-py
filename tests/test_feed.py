...
def new_client(httpserver):
    return Client(
        "dummy_api_key", host="http://" + httpserver.host + ":" + str(httpserver.port)
    )
...
def fixture_feed_response(httpserver):
    ...
    httpserver.expect_ordered_request(
        ...
        method="GET",
        headers={"X-Apikey": "dummy_api_key"},
    ).respond_with_data(bz2.compress(b'{"type": "file", "id": "dummy_file_id_4"}'))
...
def fixture_feed_response_missing_packages(httpserver):
    httpserver.expect_ordered_request(
        ...
        method="GET",
        headers={"X-Apikey": "dummy_api_key"},
    ).respond_with_data(bz2.compress(b'{"type": "file", "id": "dummy_file_id_1"}'))
    ...
    httpserver.expect_ordered_request(
        ...
        method="GET",
        headers={"X-Apikey": "dummy_api_key"},
    ).respond_with_data(bz2.compress(b'{"type": "file", "id": "dummy_file_id_2"}'))
...
def test_tolerance(
    httpserver, feed_response_missing_packages, tolerance
...
def test_cursor(
    httpserver, feed_response, test_iters, expected_cursor
):  # pylint: disable=unused-argument
    """Tests feed's cursor."""
    with new_client(httpserver) as client:
        feed = client.feed(FeedType.FILES, cursor="200102030405")
...
def test_skip(
    httpserver, feed_response, cursor, expected_file_id
):  # pylint: disable=unused-argument
