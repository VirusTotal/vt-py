...
class Client:
    ...
    def __init__(
        self, apikey, agent="unknown", host=None, trust_env=False, timeout=300
    ):
    ...
    async def get_async(self, path, *path_args, params=None):
        ...
        return ClientResponse(
            await self._get_session().get(
                self._full_url(path, *path_args), params=params
            )
        )
    ...
    async def patch_async(self, path, *path_args, data=None):
        ...
        return ClientResponse(
            await self._get_session().patch(self._full_url(path, *path_args), data=data)
        )
    ...
    async def post_async(self, path, *path_args, data=None):
        ...
        return ClientResponse(
            await self._get_session().post(self._full_url(path, *path_args), data=data)
        )
